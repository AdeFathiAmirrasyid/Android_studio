...
class MultiColor with ChangeNotifier {
  ...
  initialData() async {
    Uri url = Uri.parse(
        'https://http-req-a4c4f-default-rtdb.firebaseio.com/colors.json');
    try {
      ...
      if (hasil.statusCode >= 200 && hasil.statusCode < 300) {
        ...
        hasilData.forEach((key, value) {
          _colors.add(SingleColor(
              id: key, title: value["title"], status: value["status"]));
        });
        notifyListeners();
      } else {
  ...
  void addColor(String title) async {
    ...
    try {
      var hasil = await http.post(url,
          body: json.encode({"title": title, "status": false}));
      if (hasil.statusCode >= 200 && hasil.statusCode < 300) {
        _colors.add(SingleColor(
            id: json.decode(hasil.body)['name'].toString(), title: title));
        notifyListeners();
      } else {
        throw (hasil.statusCode);
      }
  ...
  void deleteColor(String id) async {
    Uri url = Uri.parse(
        'https://http-req-a4c4f-default-rtdb.firebaseio.com/colors/$id.json');
